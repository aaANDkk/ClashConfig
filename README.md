...
本配置的底层逻辑是：**稳定 > 清晰 > 自动化**。
不追求臃肿无用的千万条规则，
针对日常高频使用的海内外主流服务配置了 **独立策略组**，支持一键快速切换出口：

> 🧩   `AI` · `TikTok` · `动漫` · `流媒体` · `其他`

*💡 特别针对二次元受众与流媒体，合并并优化了 `动漫` 与 `流媒体` 策略组。全面覆盖巴哈姆特、Niconico、Netflix、YouTube、Disney+、Emby 等，并在规则中手动附加了次元城动漫（cycani）、AGE动漫（agedm）等的域名分流，以解决部分动漫网站屏蔽日本IP的问题。*

---

## 🛡️ 广告拦截与防护

配置中集成了轻量化的广告拦截规则集（`秋风广告`），并独设 **`广告净化`** 核心控制策略组。支持切换拦截强度，满足不同场景下的去广告需求：
...
配置顶部引入了模块化 **声明锚点** 机制，实现订阅源（Proxy Providers）与策略组的解耦。

你只需在顶层追加或修改 **订阅链接**，策略组即可实现自动加载：
...
* 使用前请自行在 `proxy-providers` 节点填写个人的 **有效订阅**。
