...
一个针对 **高阶开发者与多 AI 工具链、二次元追番生态** 深度调优的 Mihomo (Clash Meta) 核心配置文件。偏向 **极端稳定、结构解耦、低维护成本**，完美适配 TUN 模式与全端口嗅探。
...
| 🇨🇳 **台湾** | `台湾-自动选择` | 台湾省专属节点，完美解锁巴哈姆特动画疯及本地流媒体 |
| 🇯🇵 **日本** | `日本-自动选择` | 日区原生落地、Niconico 及主流二次元流媒体 |
| 🇰🇷 **韩国** | `韩国-自动选择` | 韩区特定游戏、高带宽落地服务 |
| 🇸🇬 **新加坡** | `新加坡-自动选择` | 狮城低延迟、高并发国际大厂服务 |
| 🇺🇸 **美国** | `美国-自动选择` | 美区全服务兼容，大模型与海外本土服务生态 |
| 🌍 **其他地区** | `其他地区-自动选择` | 自动正则过滤主流六区后的冷门/欧洲节点 |
| 🔓 **解锁节点** | `解锁节点-自动选择` | 智能提取包含原生、家宽、住宅及 AI/Netflix 专用解锁节点 |
...
> 🧩 `人工智能` · `Google全家桶` · `YouTube` · `TikTok` · `Anime (动画疯/动漫狂/特定追番App)` · `NETFLIX` · `Niconico` · `其他`

*💡 特别针对二次元受众，合并并优化了 `Anime` 策略组，全面覆盖巴哈姆特、动漫狂（cycani）、AGE动漫（agedm），并对 Kazumi、弹弹play 等本地播放器进程进行了完美分流寻址。*
...
* **缓存算法:** 采用高级 `arc` 缓存替换算法，兼顾高命中率与极低内存开销。
...
* 精准配置 `skip-domain` 豁免名单（如米家云、Apple PUSH 以及本地、下载类规则集），完美平衡动态嗅探与本地低延迟。
...
* **严格路由阻断:** 针对 Google 系服务执行高级 UDP 拦截（对 `google` 规则集强行 `REJECT-DROP` 443 端口），强迫流量平滑回落至 TCP 轨道，大幅提升 YouTube 等服务的抗丢包稳定性。
...
配置顶部引入了模块化 **YAML 声明锚点** 机制，实现了订阅源（Proxy Providers）与策略组的彻底解耦。同时引入了 **全局节点过滤器**，从源头过滤掉机场的通知、官网、剩余流量等无效节点。
...
proxy-providers:
  P1:
    <<: [*p, *global_exclude_filter] # 继承健康检查与全局流量节点过滤
    url: "填入订阅"     # 订阅1
    override:
...
* 🌐 全局网页及学术文献高流畅检索
* 💻 GitHub 协作、AI 辅助编程（Devv/OpenRouter/Claude）与全栈工具链平稳运行
* 📺 国际流媒体与御宅追番（Netflix/巴哈姆特/YouTube 4K）超低延迟就绪
