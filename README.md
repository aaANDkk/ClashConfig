...
偏向 **稳定、解耦、低维护成本** 的个人自用 Mihomo / Clash Meta 配置方案。
* **📝 YAML 配置**：开箱即用，支持多订阅聚合与自定义规则。
* **⚙️ JS 预处理脚本**：支持远程 URL 链式加载，无需手动反复合并订阅。
---
## ✨ 配置亮点
### 🗺️ 精准地域与隐式测速
采用 **显式手动选择** 与 **后台隐式自动测速 (`url-test`)** 联动设计，自动测速组默认过滤低倍率及部分高损耗协议。

| 节点分组 | 自动测速组 | 适用场景 |
| :--- | :--- | :--- |
| **Residential** 🏠 | `RSD-Auto` | 家宽解锁与风控服务 |
| **Hong Kong** 🇭🇰 | `HKG-Auto` | 低延迟通用出口 |
| **Taiwan** 🇨🇳 | `TWN-Auto` | 巴哈姆特等台区限制内容 |
...
proxy-providers:
  Provider_A:
    ...
    url: "https://your-subscription-url-1"
    override:
      additional-prefix: "① "
  Provider_B:
    <<: [*p, *global_exclude_filter]
...
```
> **注意**：必须使用 `raw.githubusercontent.com` 直链。若客户端不支持远程脚本，可直接复制脚本源码覆盖至本地配置预处理中。
---
## ⚠️ 免责声明
* 本项目仅为配置逻辑共享，**不提供任何节点或机场订阅**。
* 分流规则高度贴合个人开发与追番习惯，使用前请根据自身网络环境微调 `nameserver-policy` 及自定义规则集。
---
## 💖 鸣谢
